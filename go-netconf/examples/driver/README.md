...
This functionality was added to the go-netconf library so that each of the transport complies with a minimum amount of functionality. Therefore, usage is easier and you do not have to poke about in the session or transport layers directly. Each transport added exposes the same methods through an interface, that means setting it up and driving it is a piece of cake.

Here is the source code from a real client that uses the driver mechanism.
